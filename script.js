...
const generateNewQuote = arr => {
    const beginning = arr.beginnings[(Math.floor(Math.random() * arr.beginnings.length))];
    const middle = arr.middles[(Math.floor(Math.random() * arr.middles.length))];
    const ending = arr.endings[(Math.floor(Math.random() * arr.endings.length))];
